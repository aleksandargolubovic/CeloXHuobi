	...
	requests: [
		{
			...
			status: 'Pending',
			category: 'equipment',
			receipt: 'assets/images/notes/road-trip.jpeg',
			description: 'Burger',
		},
		{
			id: '2',
			creator: '70d4d7d0',
			amount: '45.63',
			date: '2016/04/23 02:07:59',
			status: 'Approved',
			category: 'travel',
			receipt: 'assets/images/notes/road-trip.jpeg',
			description: '12345qwerty',
		},
		{
			id: '3',
			creator: '70d4d7d0',
			amount: '45.63',
			date: '2019/04/23 02:07:59',
			status: 'Denied',
			category: 'meal',
			receipt: 'assets/images/notes/road-trip.jpeg',
			description: 'asdfgh765',
		},
	]
};
...
mock.onPost('/api/expensedao/request/save').reply(request => {
	const data = JSON.parse(request.data);
	let req = null;

	eCommerceDB.requests = eCommerceDB.requests.map(_request => {
	...
	if (!req) {
		req = data;
		eCommerceDB.requests = [...eCommerceDB.requests, req];
	}

	return [200, req];
});

mock.onGet('/api/expensedao/requests').reply(() => {
	console.log(eCommerceDB.requests);
	return [200, eCommerceDB.requests];
});

mock.onGet('/api/expensedao/request').reply(request => {
	const { requestId } = request.params;
	const response = _.find(eCommerceDB.requests, { id: requestId });
	return [200, response];
});
